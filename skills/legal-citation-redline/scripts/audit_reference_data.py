...
def check_rule_index(root: Path, audit: Audit) -> list[dict[str, Any]]:
    ...
    if data.get("count") != 100:
        audit.error(f"rule index count must be 100, got {data.get('count')!r}")
    ...
    if nums != list(range(1, 101)):
        missing = sorted(set(range(1, 101)) - set(n for n in nums if isinstance(n, int)))
        duplicates = sorted({n for n in nums if nums.count(n) > 1})
        audit.error(f"rule numbers must be exactly 1..100; missing={missing}, duplicates={duplicates}")
...
def check_markdown_index(root: Path, audit: Audit) -> None:
    ...
    missing = sorted(set(range(1, 101)) - found)
    if missing:
