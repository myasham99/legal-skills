...
CATEGORY_RANGES: list[tuple[str, range]] = [
    ("general_rules", range(1, 24)),
    ("print_publications", range(24, 47)),
    ("online_and_media", range(47, 52)),
    ("unpublished_conference_thesis_archive", range(52, 58)),
    ("legal_and_official_documents", range(58, 70)),
    ("judicial_cases", range(70, 74)),
    ("statistics", range(74, 77)),
    ("english_and_foreign_general", range(77, 101)),
]
...
def extract_toc_titles(lines: list[str]) -> tuple[dict[int, str], int]:
    ...
    for line_number, line in enumerate(lines[:2200], start=1):
        ...
        if 1 <= rule_number <= 100 and rule_number not in toc:
...
def find_heading_candidates(lines: list[str], min_line: int) -> dict[int, list[Candidate]]:
    candidates: dict[int, list[Candidate]] = {num: [] for num in range(1, 101)}
...
def choose_headings(
    ...
    for rule_number in range(1, 101):
...
def detect_toc_block_end(lines: list[str]) -> int:
    """Fallback TOC detector for OCR copies whose 目次 lines carry no page number.

    The handbook 目录 lists rules 1..N in order (section headings such as
    "一、" / "（一）" are interleaved and must not break the run). The body also
    repeats 1..N, so only the FIRST run reaching 50 consecutive rule numbers is
    the 目录. Return the line number of that run's last line (0 when not found).
    """
    prev_num = None
    run_start = None
...
def build_rule_index(markdown: str) -> tuple[dict[str, Any], str, list[str]]:
    ...
    if len(toc_titles) >= 80:
        min_body_line = last_toc_line + 20
    else:
        fallback_end = detect_toc_block_end(lines)
        min_body_line = fallback_end + 1 if fallback_end else 0
...
2. `handbook_rule_index.json` must contain exactly 100 rules.
3. Rule numbers must be exactly 1 through 100, with no gaps or duplicates.
4. Every rule object must contain:
   ...
   - `raw_start_line`
   - `raw_end_line`
5. Use these category ranges exactly (《法学引注手册》2019 年 11 月版，共 100 条):
   - 1-23: `general_rules`
   - 24-46: `print_publications`
   - 47-51: `online_and_media`
   - 52-57: `unpublished_conference_thesis_archive`
   - 58-69: `legal_and_official_documents`
   - 70-73: `judicial_cases`
   - 74-76: `statistics`
   - 77-100: `english_and_foreign_general`
