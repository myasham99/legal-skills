...
PDF = ROOT / "assets" / "Law_Journal_Citation_Handbook_2019.pdf"
...
def test_rule_index_exhaustive():
    ...
    assert_true(data["count"] == 100, "rule count must be 100")
    assert_true(data["missing_rule_numbers"] == [], "no rule numbers may be missing")
    assert_true(nums == list(range(1, 101)), "rule numbers must be exactly 1..100")
    for rule in rules:
